...
/** Returns true if at least `n` values in `arr` are truthy. */
function isTrueMinimumNumberOfTimes(arr: (boolean | undefined | null)[], n: number): boolean {
  return arr.filter(Boolean).length >= n;
}

/** Deep-merges `source` into `target`, adding only keys that are absent in `target`. */
function objMergeNewKey(target: Record<string, any>, source: Record<string, any>): Record<string, any> {
...
function generateSidebarItem(
  ...
  const filesByGlobPattern: string[] = fg.sync('*', {
    cwd: currentDir,
