...
function isTocSidebarDirectoryEntry(
  value: unknown,
): value is TocSidebarDirectoryEntry {
  if (!isRecord(value)) {
    return false;
  }

  const hasValidIndexFile =
    value.indexFile === null || isTocSidebarFileEntry(value.indexFile);
  return (
    typeof value.name === "string" &&
    typeof value.path === "string" &&
...
function isTocSidebarRawTreeNode(
  value: unknown,
): value is TocSidebarRawTreeNode {
  if (!isRecord(value)) {
    return false;
  }
...
function isTocSidebarDoctreePayload(
  value: unknown,
): value is TocSidebarDoctreePayload {
...
async function loadDoctreeData(): Promise<TocSidebarRawTree | null> {
  ...
  try {
    // 动态导入虚拟模块，避免初始化时加载
    const doctreeModule =
      await import("virtual:@knewbeing/toc-sidebar-doctree");
...
function getCurrentRouteTree(
  sourceRawTree: TocSidebarRawTree | null,
  routePath: string,
): AutoTocRouteTreeEntry[] {
  if (!sourceRawTree) {
    return [];
...
function pickCurrentLevelEntries(
  entries: AutoTocRouteTreeEntry[],
): AutoTocEntry[] {
  return entries
...
const siblingEntries = computed<DisplayEntry[]>(() => {
  ...
  const siblings =
    currentRouteTree.length > 0
      ? pickCurrentLevelEntries(currentRouteTree)
      : [];
  ...
  return siblings
    .map((item) => {
      ...
      const kind: DisplayEntry["kind"] = isDirectoryLink(link)
        ? "directory"
        : "file";
      return {
        text,
...
<template>
  <nav
    v-if="groupedEntries.directories.length || groupedEntries.files.length"
    class="auto-toc"
    aria-label="同级目录"
  >
    ...
    <section
      v-if="groupedEntries.files.length"
      class="auto-toc__section auto-toc__section--file"
    >
      ...
      <ul class="auto-toc__list">
        <li
          v-for="item in groupedEntries.files"
          :key="item.link"
    ...
    <section
      v-if="groupedEntries.directories.length"
      class="auto-toc__section auto-toc__section--dir"
    >
      <h3 class="auto-toc__section-title">
      ...
      <ul class="auto-toc__list">
        <li
          v-for="item in groupedEntries.directories"
          :key="item.link"
          class="auto-toc__item"
        >
          <a
