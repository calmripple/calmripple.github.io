...
![图片](./assets/tx-serverless-static-001.png)

然后在左上角的云产品列表中找到[云开发CloudBase](https://console.cloud.tencent.com/tcb/env/index?rid=4)

![图片](./assets/tx-serverless-static-002.png)

然后你能看到如下的界面

![图片](./assets/tx-serverless-static-003.png)

点击新建，可以看见有很多模板可供选择，咱们这里选择Vue应用，当然你也可以选择其它模板，我们主要是获得其生成的一个`cloudbaserc.json`文件，后文会详细介绍这个配置文件

![图片](./assets/tx-serverless-static-004.png)

填写环境名称，选择按量计费（非常非常便宜），勾上**同意**，能勾上免费就勾上**免费资源**

填写完成后点击下一步

![图片](./assets/tx-serverless-static-005.png)

点击立即开通

![图片](./assets/tx-serverless-static-006.png)

接下来你能在当前页面看到刚刚创建的应用，记住这个环境id 后面会用上

![图片](./assets/tx-serverless-static-007.png)
...
![图片](./assets/tx-serverless-static-008.png)

选择左侧菜单中的静态网站托管

![图片](./assets/tx-serverless-static-009.png)
...
node -v
```
![图片](./assets/tx-serverless-static-010.png)
...
tcb -v
```
![图片](./assets/tx-serverless-static-011.png)
...
![图片](./assets/tx-serverless-static-012.png)
...
![图片](./assets/tx-serverless-static-013.png)
...
![图片](./assets/tx-serverless-static-014.png)

选择免费的

![图片](./assets/tx-serverless-static-015.png)

然后下一步填写必要的信息

![图片](./assets/tx-serverless-static-016.png)

选择DNS验证

![图片](./assets/tx-serverless-static-017.png)

按要求添加一条解析规则

![图片](./assets/tx-serverless-static-018.png)

我这里域名是在阿里云上购买的

![图片](./assets/tx-serverless-static-019.png)

按要求添加解析

![图片](./assets/tx-serverless-static-020.png)

添加完成后，点击查询域名验证状态即可，申请完成后如下图所示

![图片](./assets/tx-serverless-static-021.png)

### 绑定域名

在静态网站托管tab 基础配置中添加自定义域名

![图片](./assets/tx-serverless-static-022.png)

弹窗中输入刚刚申请证书用的域名

![图片](./assets/tx-serverless-static-023.png)

根据要求添加CName

![图片](./assets/tx-serverless-static-024.png)

上述添加解析步骤一样

![图片](./assets/tx-serverless-static-025.png)
...
![图片](./assets/tx-serverless-static-026.png)
...
![图片](./assets/tx-serverless-static-027.png)

![图片](./assets/tx-serverless-static-028.png)
...
![图片](./assets/tx-serverless-static-029.png)

添加完成后如下图所示

![图片](./assets/tx-serverless-static-030.png)
...
![图片](./assets/tx-serverless-static-031.png)

在Action面板中也能看见任务的执行状态，点击可查看详细进度

![图片](./assets/tx-serverless-static-032.png)

执行完成后如下图所示

![图片](./assets/tx-serverless-static-033.png)
## 参考资料
