...
🧪 【中文注释】
  工具名称: Vue Pinia单
  功能分类: 测试辅助
  功能说明: 帮助编写和审查 Vue 3 + TypeScript + Pinia 的单元测试，支持组件、状态管理和黑盒测试。
  使用方式: 在 VS Code 中通过 Copilot Chat 输入 /unit-test-vue-pinia 命令调用，或在 GitHub PR 审查时使用。
  关键标签: 测试、Vue、Pinia、单元测试
-->
